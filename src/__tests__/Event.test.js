...
describe('Event /> component', () => {
  ...
  test('render default info in event item', () => {
    expect(EventWrapper.find('.event-default-info')).toHaveLength(1);
  });
  ...
  test('render event info correctly', () => {
    expect(EventWrapper.find('.event-default-info').text()).toContain(
      event.start.dateTime
    );
    expect(EventWrapper.find('.event-default-info').text()).toContain(
      event.start.timeZone
    );
    expect(EventWrapper.find('.event-default-info').text()).toContain(event.location);
  });
